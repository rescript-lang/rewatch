...
pub fn validate_packages_dependencies(packages: &AHashMap<String, Package>) -> bool {
    for (package_name, package) in packages {
        ...
        if let Some(unallowed_dependency) = get_unallowed_dependents(packages, package_name, bs_dependencies)
        {
            println!(
                ...
                console::style(unallowed_dependency).bold(),
                console::style(String::from("allowed_dependents")).bold().dim()
            );
            return false;
        }
        if let Some(unallowed_dependency) =
            get_unallowed_dependents(packages, package_name, pinned_dependencies)
        {
            println!(
                "\n{} the package {} is not allowed to have {} as a pinned-dependency. Check the {} entry in the bsconfig.json",
                console::style("Error").red(),
                console::style(package_name).bold(),
                console::style(unallowed_dependency).bold(),
                console::style(String::from("allowed_dependents")).bold().dim()
        ...
        if let Some(unallowed_dependency) = get_unallowed_dependents(packages, package_name, dev_dependencies)
        {
            println!(
                "\n{} the package {} is not allowed to have {} as a dev-dependencies. Check the {} entry in the bsconfig.json",
                console::style("Error").red(),
                console::style(package_name).bold(),
                console::style(unallowed_dependency).bold(),
                console::style(String::from("allowed_dependents")).bold().dim()
