...
fn remove_ast(source_file: &str, package: &packages::Package) {
...
fn remove_iast(source_file: &str, package: &packages::Package) {
    let _ = std::fs::remove_file(helpers::get_compiler_asset(
        package,
        source_file,
        &packages::Namespace::NoNamespace,
...
pub fn remove_compile_assets(
    ...
    for extension in &["cmj", "cmi", "cmt", "cmti"] {
        remove_compile_asset(source_file, package, namespace, extension);
    }
}
...
pub fn cleanup_previous_build(
    ...
    let deleted_interfaces = diff
        ...
        .map(|res_file_location| {
            ...
            let package = build_state.get_package(package_name).unwrap();
            remove_compile_assets(res_file_location, package, package_namespace);
            remove_mjs_file(
                &res_file_location,
                &suffix.to_owned().unwrap_or(bsconfig::Suffix::Mjs),
            );
            remove_iast(res_file_location, package);
            remove_ast(res_file_location, package);
...
pub fn cleanup_after_build(build_state: &BuildState) {
    build_state.modules.par_iter().for_each(|(_module_name, module)| {
        ...
        if failed_to_parse(module) {
            match &module.source_type {
                SourceType::SourceFile(source_file) => {
                    remove_iast(&source_file.implementation.path, package);
                    remove_ast(&source_file.implementation.path, package);
                }
                _ => (),
            }
        }
